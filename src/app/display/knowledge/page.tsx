...
export default function Bee_education() {
  const [know, setKnow] = useState<KnowData[]>([]);
  ...
  useEffect(() => {
    const fetchStories = async () => {
      setLoading(true);
      try {
        const response = await axiosInstance.get("/get_all_education");
                      ...
                      {know.map((know, index) => (
                        <SwiperSlide key={index}>
                          <div className="mb-4 p-4 border rounded-lg bg-white shadow">
                            <p className="text-xl font-semibold text-amber-700">{know.name}</p>
                            <p className="mt-2 text-gray-700">{know.education}</p>
                          </div>
                        </SwiperSlide>
                      ))}
                    </Swiper>
